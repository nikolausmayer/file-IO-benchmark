...
if test -d tinyrandom; then
  ...
  echo "Generating \"tinyrandom\" dataset... (ca. 50 Gigabytes)";
  mkdir tinyrandom;
  pushd tinyrandom;
  $DEBUG "$FOLDER"/example-data/make-random-example-files.sh --file-size 50K --number-of-files 1000000;
  popd;
fi
...
if test -d bigrandom; then
  ...
  echo "Generating \"bigrandom\" dataset... (ca. 100 Gigabytes)";
  mkdir bigrandom;
  pushd bigrandom;
  $DEBUG "$FOLDER"/example-data/make-random-example-files.sh --file-size 2G --number-of-files 50;
  popd;
fi
